...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get burn down chart data via AJAX
...
<body>
    ...
    <script>
        ...
        // Update burn down chart based on selected sprint using AJAX
        function updateBurnDownChart() {
            const sprintId = document.getElementById('sprint-select').value;
            
            // Fetch new burn down data
            fetch('bmb_taskview.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: `get_burn_down_data=true&sprint_id=${sprintId}`
            })
            .then(response => response.json())
            .then(data => {
                // Update the chart with new data
                if (burnDownChart) {
                    burnDownChart.data.labels = data.labels;
                    burnDownChart.data.datasets[0].data = data.data;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            burnDownChart = new Chart(ctx, {
                type: 'line',
                data: {
                    labels: <?php echo json_encode(array_keys($burn_down_data)); ?>,
                    datasets: [{
                        label: 'Remaining Story Points',
                        data: <?php echo json_encode(array_values($burn_down_data)); ?>,
                        borderColor: '#6a11cb',
            ...
            // Simple drag and drop for Kanban view
                ...
                column.addEventListener('drop', e => {
                    ...
                    if (draggedTask) {
                        ...
                        fetch('bmb_taskview.php', {
                            ...
                            body: `update_task_status=true&task_id=${taskId}&status=${newStatus}`
                        });
                    }
                });
            });
