...
function handleDirectUpload() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
        ...
        // Remote target file path (inside htdocs/HR)
        $remote_path = 'htdocs/HR/' . $file_name;
        
        // Upload file
        if (ftp_put($conn_id, $remote_path, $file_tmp, FTP_BINARY)) {
            echo "<p style='color:green;'>✅ File uploaded successfully to <strong>htdocs/HR/$file_name</strong>.</p>";
        } else {
            echo "<p style='color:red;'>❌ Failed to upload file to htdocs/HR/.</p>";
        }
...
function handleUpload() {
    ...
    if (ftp_put($ftp_conn, $remote_path, $file_tmp, FTP_BINARY)) {
        ...
        $response['message'] = 'File uploaded successfully to htdocs/HR/'.$file_name;
...
function listFiles() {
    ...
    // List files in the htdocs/HR directory
    $files = ftp_nlist($ftp_conn, 'htdocs/HR/');
...
function getDownloadUrl() {
    ...
    $direct_url = "ftp://$ftp_username:$ftp_password@$ftp_hostname/htdocs/HR/" . urlencode($filename);
...
function downloadFile() {
    ...
    $remote_file_path = 'htdocs/HR/' . $filename;
