...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_exit'])) {
    ...
    // Insert exit request
    $insert_query = "INSERT INTO exit_management (employee_id, resignation_date, last_working_date, exit_reason, exit_interview, feedback) 
                     VALUES (?, ?, ?, ?, ?, ?)";
    $insert_stmt = $conn->prepare($insert_query);
    $insert_stmt->bind_param("isssss", $employee_id, $resignation_date, $last_working_date, $exit_reason, $exit_interview, $feedback);
    
    if ($insert_stmt->execute()) {
        $success_message = "Exit request submitted successfully!";
    } else {
        $error_message = "Error submitting exit request. Please try again.";
    }
}

// Get exit requests for this employee
$exit_query = "SELECT * FROM exit_management WHERE employee_id = ? ORDER BY created_at DESC";
$exit_stmt = $conn->prepare($exit_query);
$exit_stmt->bind_param("i", $employee_id);
$exit_stmt->execute();
...
<head>
    ...
    <title>Employee Exit Request - Buymeabook</title>
...
<body class="expanded">
    ...
    <div class="nav-pane expanded">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item">
                <a href="employee_exit_request.php" class="nav-link active">
    ...
    <div class="container">
        <div class="content-card">
            ...
            <p class="subtitle">Submit and track your exit requests</p>
            ...
            <div class="warning-box">
                ...
                <p>Submitting an exit request is a serious action. Please ensure you have discussed this with your manager and HR before proceeding. Once submitted, your request will be reviewed by the HR department.</p>
            </div>
            
            <h2 class="section-title">Exit Request Form</h2>
            <form method="POST" action="">
                ...
                <div class="form-group">
                    <label for="exit_reason">Reason for Exit</label>
                    <select id="exit_reason" name="exit_reason" required>
                ...
                <div class="form-group">
                    <label for="exit_interview">Exit Interview Comments</label>
                    <textarea id="exit_interview" name="exit_interview" placeholder="Please share your experience working with us and any suggestions for improvement"></textarea>
                </div>
                
                <div class="form-group">
                    <label for="feedback">Additional Feedback</label>
                    <textarea id="feedback" name="feedback" placeholder="Any other feedback you would like to share"></textarea>
                </div>
                
                <button type="submit" name="submit_exit" class="btn btn-primary">
                    <i class="fas fa-paper-plane"></i> Submit Exit Request
                </button>
            </form>
            
            <h2 class="section-title">Your Exit Requests</h2>
            
            <?php if ($exit_result->num_rows > 0): ?>
                <table class="exit-table">
                    ...
                    <tbody>
                        <?php while ($exit = $exit_result->fetch_assoc()): ?>
                            <tr>
                                <td><?php echo date('M j, Y', strtotime($exit['resignation_date'])); ?></td>
                                <td><?php echo date('M j, Y', strtotime($exit['last_working_date'])); ?></td>
                                <td><?php echo htmlspecialchars($exit['exit_reason']); ?></td>
                                <td>
                                    <?php 
                                    $status_class = '';
                                    switch ($exit['rehire_eligibility']) {
                                        case 'Eligible':
                                    ...
                                    ?>
                                    <span class="<?php echo $status_class; ?>"><?php echo $exit['rehire_eligibility']; ?></span>
                                </td>
                                <td>
                                    <?php 
                                    if ($exit['rehire_eligibility'] == 'To be Decided') {
                                        echo '<span class="status-tbd">Under Review</span>';
                                    } else {
                                        echo '<span class="status-eligible">Processed</span>';
                                    }
                                    ?>
                                </td>
                                <td><?php echo date('M j, Y g:i A', strtotime($exit['created_at'])); ?></td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
            <?php else: ?>
                <p>You haven't submitted any exit requests yet.</p>
            <?php endif; ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const lastWorkingDate = document.getElementById('last_working_date');
            
            if (resignationDate && lastWorkingDate) {
                resignationDate.addEventListener('change', function() {
                    if (lastWorkingDate.value && new Date(resignationDate.value) > new Date(lastWorkingDate.value)) {
                        alert('Resignation date cannot be after last working date');
                        resignationDate.value = '';
                    }
                });
                
                lastWorkingDate.addEventListener('change', function() {
                    if (resignationDate.value && new Date(lastWorkingDate.value) < new Date(resignationDate.value)) {
                        alert('Last working date cannot be before resignation date');
                        lastWorkingDate.value = '';
                    }
                    
                    // Check if last working date is at least 2 weeks after resignation date
                    if (resignationDate.value) {
                        const resignation = new Date(resignationDate.value);
                        const lastWorking = new Date(lastWorkingDate.value);
                        const twoWeeksLater = new Date(resignation);
                        twoWeeksLater.setDate(twoWeeksLater.getDate() + 14);
                        
                        if (lastWorking < twoWeeksLater) {
                            if (!confirm('Your last working date is less than 2 weeks from your resignation date. Are you sure you want to proceed?')) {
                                lastWorkingDate.value = '';
                            }
                        }
                    }
            ...
            // Confirm before submitting exit request
            const exitForm = document.querySelector('form');
            if (exitForm) {
                exitForm.addEventListener('submit', function(e) {
                    if (!confirm('Are you sure you want to submit this exit request? This action cannot be undone.')) {
                        e.preventDefault();
                    }
                });
            }
